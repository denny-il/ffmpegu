...
- [2 Syntax](#2-syntax)
    - [2.1 Quoting and escaping](#21-quoting-and-escaping)
    - [2.2 Date](#22-date)
    - [2.3 Time duration](#23-time-duration)
    - [2.4 Video size](#24-video-size)
    - [2.5 Video rate](#25-video-rate)
    - [2.6 Ratio](#26-ratio)
    - [2.7 Color](#27-color)
    - [2.8 Channel Layout](#28-channel-layout)
- [3 Expression Evaluation](#3-expression-evaluation)
...
*   **bitand(x, y)**, **bitor(x, y)**: Compute bitwise and/or operation on *x* and *y*. The results are converted to integers before executing the bitwise operation. Precision may be lost for large numbers (usually 2^53 and larger).
*   **ceil(expr)**: Round the value of expression *expr* upwards to the nearest integer.
...
*   **exp(x)**: Compute exponential of *x* (with base `e`).
*   **floor(expr)**: Round the value of expression *expr* downwards to the nearest integer.
...
*   **hypot(x, y)**: Returns `sqrt(x*x + y*y)`, the length of the hypotenuse or distance from the origin.
*   **if(x, y)**: Evaluate *x*, and if the result is non-zero return the result of *y*, return 0 otherwise.
*   **if(x, y, z)**: Evaluate *x*, and if the result is non-zero return *y*, otherwise return *z*.
*   **ifnot(x, y)**: Evaluate *x*, and if the result is zero return the result of *y*, return 0 otherwise.
*   **ifnot(x, y, z)**: Evaluate *x*, and if the result is zero return *y*, otherwise return *z*.
*   **isinf(x)**: Return 1.0 if *x* is +/-INFINITY, 0.0 otherwise.
*   **isnan(x)**: Return 1.0 if *x* is NAN, 0.0 otherwise.
*   **ld(idx)**: Load the value of the internal variable with index *idx*.
...
*   **pow(x, y)**: Compute the power of *x* elevated *y*.
*   **print(t)**, **print(t, l)**: Print the value of expression *t* with loglevel *l*.
*   **random(idx)**: Return a pseudo random value between 0.0 and 1.0. *idx* is the index of the internal variable used to save the seed/state.
    *   Example to initialize seed with 42: `st(0,42); print(random(0)); print(random(0)); print(random(0))`
*   **randomi(idx, min, max)**: Return a pseudo random value in the interval between *min* and *max*.
...
*   **round(expr)**: Round the value of expression *expr* to the nearest integer.
*   **sgn(x)**: Compute sign of *x*.
*   **sin(x)**: Compute sine of *x*.
*   **sinh(x)**: Compute hyperbolic sine of *x*.
*   **sqrt(expr)**: Compute the square root of *expr*.
*   **squish(x)**: Compute expression `1/(1 + exp(4*x))`.
*   **st(idx, expr)**: Store the value of the expression *expr* in an internal variable *idx* (0 to 9).
*   **tan(x)**: Compute tangent of *x*.
*   **tanh(x)**: Compute hyperbolic tangent of *x*.
*   **taylor(expr, x)**, **taylor(expr, x, idx)**: Evaluate a Taylor series at *x*, given an expression representing the `ld(idx)`-th derivative of a function at 0.
*   **time(0)**: Return the current (wallclock) time in seconds.
*   **trunc(expr)**: Round the value of expression *expr* towards zero to the nearest integer.
*   **while(cond, expr)**: Evaluate expression *expr* while the expression *cond* is non-zero.
...
Example: `if (A AND B) then C` is equivalent to `if(A*B, C)`.

The evaluator also recognizes International System (SI) unit prefixes. If ’i’ is appended after the prefix, binary prefixes are used (powers of 1024). The ’B’ postfix multiplies the value by 8.

Available SI prefixes:
